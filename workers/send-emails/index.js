/* ============================================================
   Cairn, the sender. A Cloudflare Worker on an hourly cron.
   ...
   just fetch, so this deploys with no build step at all: it can be
   pasted straight into the Cloudflare dashboard editor.

   Secrets arrive on `env`, set under the Worker's Settings. There is
   no process.env in a Worker, and none of these belong anywhere near
   config.js, which is public.
   ============================================================ */

/* Read once per run from `env`, then passed down. A Worker has no
   ambient process.env to reach for. */
function settings(env) {
  return {
    SUPABASE_URL: env.SUPABASE_URL,
    SERVICE_KEY:  env.SUPABASE_SERVICE_ROLE_KEY,
    RESEND_KEY:   env.RESEND_API_KEY,
    FROM_EMAIL:   env.MAIL_FROM      || 'hello@praycairn.com',
    FROM_NAME:    env.MAIL_FROM_NAME || 'Cairn',
    SITE:        (env.SITE_URL || 'https://praycairn.com').replace(/\/$/, ''),
    OWNER_EMAIL:  env.OWNER_EMAIL || '',
    SEND_HOUR:    Number(env.SEND_HOUR || 7),
    DRY_RUN:      env.DRY_RUN === '1',
  };
}

/* The 260 chapter plan, inlined: a Worker pasted into the dashboard
   has no bundler to follow an import. Kept identical to
   src/readings.js; if that list ever changes, change it here too. */
const NT = [
...
const PLAN = NT.flatMap(([book, n]) => Array.from({ length: n }, (_, i) => `${book} ${i + 1}`));

/* ============================================================
   The run. Everything below closes over one config read from `env`,
   because a Worker has no ambient environment to reach for.
   ============================================================ */
async function run(env) {
  const C = settings(env);
  const results = { welcome: 0, remember: 0, checkin: 0, reading: 0, ideas: 0, errors: 0 };

  if (!C.SUPABASE_URL || !C.SERVICE_KEY || (!C.RESEND_KEY && !C.DRY_RUN)) {
    console.error('[cairn] sender is missing its environment variables');
    return { ...results, errors: 1, configured: false };
  }
  ...
  async function send({ to, name, subject, html, text, token, kind }) {
    if (C.DRY_RUN) { console.log(`[dry] ${kind} → ${to}: ${subject}`); return true; }
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        text,
        headers: {
          'List-Unsubscribe': `<${C.SITE}/api/unsubscribe?t=${token}&k=${kind}>, <mailto:${C.FROM_EMAIL}?subject=unsubscribe>`,
  ...
  <a href="${C.SITE}/unsubscribe/${token}?k=${kind}" style="color:#8A857E">Turn this email off</a>
  &nbsp;·&nbsp;
  <a href="${C.SITE}/#settings" style="color:#8A857E">All email settings</a>
  ...
  ${btn('See your answered prayers', `${C.SITE}/#answered`)}`;
    return {
  ...
  ${btn('Read it', C.SITE)}`;
    return {
      ...
      text: `Good morning ${name}.\n\nToday: ${reference}\n\n${C.SITE}\n\nTurn this off: ${C.SITE}/unsubscribe/${p.token}?k=reading`,
    };
  }
  ...
  async function forwardSuggestions(results) {
    ...
    const ok = await send({
      to: C.OWNER_EMAIL, name: 'Cairn', kind: 'product', token: 'owner',
...
export default {
  /* The hourly cron. Declared in wrangler.toml, not in code. */
  async scheduled(controller, env, ctx) {
    ctx.waitUntil(run(env));
  },

  /* A way to kick it by hand without waiting an hour. Guarded by a
     shared secret so the open internet cannot make Cairn send mail. */
  async fetch(request, env) {
    const url = new URL(request.url);
    if (url.pathname !== '/run') return new Response('cairn sender', { status: 200 });
    const given = request.headers.get('authorization') || url.searchParams.get('key') || '';
    if (!env.RUN_KEY || given.replace(/^Bearer /, '') !== env.RUN_KEY) {
